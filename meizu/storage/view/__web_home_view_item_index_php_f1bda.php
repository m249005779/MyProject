...
<head>
    ...
    <link rel="stylesheet" href="<?php echo __ROOT__?>/resource/home/css/product.css" />
...
<section>
    <div class="container">
        <div class="crumbs clearfix">
            <a href="<?php echo u('entry.index')?>" class="">首页</a>
            &nbsp;>&nbsp;
            <a class="ellipsis crumbs-title"><?php echo $goodsData['gname']?></a>
        </div>
        <div class="row">
            <div class="preview" id="preview">
                <div class="preview-booth">
                    ...
                    <a href="javascript:;" id="J_imgBooth" class="hide">
                        <img src="<?php echo $v?>" height="375" width="375"/>
                    </a>
                    <?php }?>
                </div>
                <ul class="preview-thumb clearfix" id="J_previewThumb">
                    <?php foreach ((array)$detailData['medium'] as $k=>$v){?>
                    <li>
                        <a href="javascript:;">
                            <img src="<?php echo $v?>" height="75" width="75"/>
                        </a>
                    </li>
                    <?php }?>
                </ul>
            ...
            <div class="property" id="property">
                <div class="property-hd">
                    <h1><?php echo $goodsData['gname']?></h1>
                    <p class="mod-info"><?php echo $tData['网络类型'][0]?></p>
                </div>
                ...
                <div class="property-set">
                    <?php foreach ((array)$specData as $k=>$v){?>
                    <dl class="property-set-sale">
                        <dt class="vm-metatit taname"><?php echo $v['taname']?>:</dt>
                        <dd class="clearfix net">
                            <?php foreach ((array)$v['tavalue'] as $kk=>$vv){?>
                            <a href="javascript:;" taname="<?php echo $v['taname']?>" gtid="<?php echo $vv['gtid']?>" class="">
                                <span><?php echo $vv['gtvalue']?></span>
                            </a>
                            <?php }?>
                        </dd>
                    </dl>
                    ...
                    <script>
                        $(function () {
                            $('.net a').click(function () {
                                if($('.selected').length != <?php echo count($specData)?>){
                                    return false;
                                }
                                var combine = '';
...
//                                alert(combine);
                                ...
                                $.post("<?php echo u('ajaxGetTotal')?>",{gid:gid,combine:combine},function (res) {
                                    if(res.valid){
                        ...
                        $(function () {
                           if(!$('.net a').hasClass('selected')) {
                            ...
                            $('#J_btnBuy').click(function () {
                                ...
                                $.post("<?php echo u('ajaxAddCart')?>",data,function(res){
                                    if (res.val){
                                        location="<?php echo u('order.index')?>";
                                    }else{
                                        location="<?php echo u('login.index')?>";
                                    }
                                },'json');
                            })
                        })
                            ...
                            <script>
                                $(function () {
                                    $('#J_favorite').click(function () {
                                        var gid = <?php echo q('get.gid')?>;
                                        $.post("<?php echo u('ajaxCollect')?>",{gid:gid},function (res) {
                                            if (res.val){
                                                $('.model').show();
                                            }else if(res.val==0){
                                                location="<?php echo u('login.index')?>";
...
<!--                                <li>-->
<!--                                    <a href="">-->
<!--                                        <img src="<?php echo __ROOT__?>/resource/images/item/17.jpg" width="32" height="32"/>-->
<!--                                        魅蓝note3 799元起-->
<!--                                    </a>-->
<!--                                </li>-->
<!--                                <li>-->
...
<div class="model">
    <div class="tcenter">
        <div class="alert">
            <p>
                <span>收藏成功</span>
                <a href="<?php echo u('center.collect')?>">查看我的收藏</a>
            </p>
        </div>
    </div>
