...
<head>
    ...
    <link rel="stylesheet" href="{{__ROOT__}}/resource/home/css/paycenter.css" />
...
<!--            </div>-->
    ...
    <nav>
        <div class="nav-mid">
            <div class="logo">
                <a href="">
                    <img src="{{__ROOT__}}/resource/images/entry/logo-header.png" alt="">
                </a>
            </div>
            <div class="head-nav">
                <ul>
        ...
        <div class="mzcontainer">
            <div class="payment_header clearfix">
                <div class="icon">
                    ...
                    <div class="info">
                        <div class="main">
                            <h2>订单提交成功，应付金额 <span>{{$data['totalprice']}}.00</span>元</h2>
                        ...
                        <div class="other">
                            <div class="tr">
                                ...
                                <div class="td value">
                                    <span class="prod"> {{$data['gname']}} <foreach from="$data['options']" key="$k" value="$v">{{$v}} </foreach> X {{$data['quantity']}}</span>
                                </div>
                            </div>
                            <div class="tr">
                                <div class="td name">
                                ...
                                <div class="td value">
                                    <span class="prod"> <foreach from="$address['city']" key="$k" value="$v">{{$v}}</foreach>{{$address['address']}}  {{$address['username']}}(收) {{$address['phone']}}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <script>
                    $(function () {
                        $('.go_to_pay').click(function () {
                            var oid = {{q('get.oid')}};
                            $.post("{{u('goToPay')}}",{oid:oid},function (res) {
                                if (res.val){
                                    alert('付款成功请等待发货');
                                    location="{{u('entry.index')}}";
                                }
                            },'json')
                        })
                    })
